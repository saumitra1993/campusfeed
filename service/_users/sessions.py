...
from db.database import DBMobileAuth, Users, Channels, Posts, Channel_Admins, Views, Channel_Followers, DBUserGCMId
...
class LoginRequired(object):
	...
	def __call__(self, *args, **kwargs):
		...
		try:
			...
			logging.info("Came in except")
			if obj.session.get('userid') is not None:
				logging.info(obj.session['userid'])
				r = self.f(obj, *args, **kwargs)
				return r
			elif token:
				customer = customers.get(token)
