		...
		if len(user_followed_channels)>0:	
			for user_followed_channel in user_followed_channels:	
				...
				if user.type_ == 'admin' or user.type_ == 'superuser':
					...
					posts_query = Posts.query(Posts.channel_ptr == channel_ptr)
					if len(is_admin) == 1:
						admin_bool = 1

				if user.type_ == 'user':
					posts_query = Posts.query(ndb.OR(ndb.AND(Posts.channel_ptr == channel_ptr, Posts.pending_bit == 0), ndb.AND(Posts.channel_ptr == channel_ptr, Posts.user_ptr == user.key, Posts.pending_bit == 1)))
